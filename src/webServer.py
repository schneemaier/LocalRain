...
# This flag enables the use of a less restrictive, but slower decoder
...
def update_states(bin_state, remote_id):
    ...
    unit.append(hex(bin_state[bin_fields['UNIT_ID_HIGH_1']])[2:] + hex(bin_state[bin_fields['UNIT_ID_LOW_1']])[2:])
        ...
        for i in range(2):
            for j in range(4):
                try:
                    ...
                    # if valve is not in the list use 0 minutes (do nothing)
                    struct.pack_into('<H', buffer, 2 + 2 * j + i * 10, 0)
...
async def handle_submit(request):
    ...
    if message.endswith('ack--null'):
        ...
        padded_message = message.replace("-", "+") + '=' * (-len(message) % 4)
...
async def all_timestamp_loop():
    ...
    while True:
        ...
        if len(time_stamp) != 0:
            ...
            for key in channels:
                channels[key] = minutes_of_day
                await msg_timestamp(minutes_of_day, now.weekday(), key)
        else:
            sleeptime = 60
        # send timestamp every 30 miuntes for all connected devices
    ...
    while True:
        ...
        if len(time_stamp) != 0:
            for key in time_stamp:
                logger.info(f'Global watchdog loop {f}', key)
